...
export type RateLimitedErrorData = {
   ...
   */
  title: string;
  status: models.RateLimitedErrorStatus;
  /**
  ...
  code?: models.RateLimitedErrorCode | undefined;
  /**
   * Seconds until the next request may succeed.
   */
 ...
 * RFC 9457 Problem Details error response. All error responses follow this format. Match on `code` for programmatic handling; per-status narrowed schemas allOf this base.
 */
export class RateLimitedError extends AuditLockerError {
  /**
   * URI reference identifying the error type
   */
  type: string;
...
> = z.pipe(
  z.object({
    type: types.string(),
    title: types.string(),
    status: models.RateLimitedErrorStatus$inboundSchema,
    detail: types.optional(types.string()),
    instance: types.optional(types.string()),
    code: types.optional(models.RateLimitedErrorCode$inboundSchema),
