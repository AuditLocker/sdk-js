...
export type NotFoundErrorData = {
   ...
   */
  title: string;
  status: models.NotFoundErrorStatus;
  /**
  ...
  code?: models.NotFoundErrorCode | undefined;
};

/**
 * RFC 9457 Problem Details error response. All error responses follow this format. Match on `code` for programmatic handling; per-status narrowed schemas allOf this base.
 */
export class NotFoundError extends AuditLockerError {
  /**
   * URI reference identifying the error type
   */
  type: string;
...
> = z.pipe(
  z.object({
    type: types.string(),
    title: types.string(),
    status: models.NotFoundErrorStatus$inboundSchema,
    detail: types.optional(types.string()),
    instance: types.optional(types.string()),
    code: types.optional(models.NotFoundErrorCode$inboundSchema),
