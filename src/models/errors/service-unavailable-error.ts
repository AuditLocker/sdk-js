...
export type ServiceUnavailableErrorData = {
   ...
   */
  title: string;
  status: models.ServiceUnavailableErrorStatus;
  /**
  ...
  code?: models.ServiceUnavailableErrorCode | undefined;
};

/**
 * RFC 9457 Problem Details error response. All error responses follow this format. Match on `code` for programmatic handling; per-status narrowed schemas allOf this base.
 */
export class ServiceUnavailableError extends AuditLockerError {
  /**
   * URI reference identifying the error type
   */
  type: string;
...
> = z.pipe(
  z.object({
    type: types.string(),
    title: types.string(),
    status: models.ServiceUnavailableErrorStatus$inboundSchema,
    detail: types.optional(types.string()),
    instance: types.optional(types.string()),
    code: types.optional(models.ServiceUnavailableErrorCode$inboundSchema),
