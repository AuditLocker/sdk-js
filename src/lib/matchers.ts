...
import { AuditLockerDefaultError } from "../models/errors/audit-locker-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AuditLockerDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AuditLockerDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AuditLockerDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AuditLockerDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
